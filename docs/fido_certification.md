...
| Parameter                        | Implementation        | Purpose                                                               | Secret | Strength | Storage            | Protection             | Generation                             | Input / Output                | Deleted                                     | Sharing                |
|----------------------------------|-----------------------|-----------------------------------------------------------------------|--------|----------|--------------------|------------------------|----------------------------------------|-------------------------------|---------------------------------------------|------------------------|
| Authenticator Application        | Javacard CAP file     | Execute in ROE to provide functionality                               | No     | N/A      | Flash              | ROE                    | By vendor when building applet         | Vendor in, never out          | Applet uninstallation                       | Global                 |
| User PIN derivative              | LEFT16(SHA-256(PIN))  | Determine if a pinUvAuthToken should be granted                       | Yes    | 128      | None               | User                   | By user when setting/changing PIN      | CTAP in, never out            | N/A (not stored)                            | N/A                    |
| PIN derivation salt              | 28-byte random value  | Derive HMAC key from user PIN                                         | No     | 224      | Flash              | ROE                    | By ROE at applet install/reset         | Never                         | CTAP reset                                  | Per authenticator      |
| PIN Verification Nonce           | 32-byte random value  | Verify the correct PIN was provided by the user using HMAC            | No     | N/A      | Flash              | ROE                    | By ROE at applet install/reset         | Never                         | CTAP pin set/change                         | Per authenticator      |
| CTAP PIN Protocol 2 IVs          | 16-byte values        | Prevent replay/modification of commands protected by PIN protocol 2   | No     | N/A      | None               | Not stored             | By ROE at CTAP PIN operation           | CTAP                          | N/A (not stored)                            | Per command            |
| Credential Wrapping cp3 UV Key   | 32-byte random value  | Encrypt/decrypt KeyHandles using credProtect level 3 with PIN         | Yes    | 128[3]   | Flash              | ROE+AES256             | By ROE at applet install/reset         | Never                         | CTAP reset                                  | Per authenticator      |
| Credential Wrapping Non-UV Key   | 32-byte random value  | Encrypt/decrypt KeyHandles credProtect<3 or no PIN                    | Yes    | 256      | Flash              | ROE+AES256             | By ROE at applet install/reset         | Never                         | CTAP reset                                  | Per authenticator      |
| Credential Wrapping HMAC Key     | 32-byte random value  | Verify Credential IDs                                                 | Yes    | 128      | Flash              | ROE+AES256             | By ROE at applet install/reset         | Never                         | CTAP reset                                  | Per authenticator      |
| CTAP Platform Agreement Key      | EC key on P-256       | ECDH-generate a shared secret between Authenticator and Platform      | Yes    | 128      | RAM                | ROE                    | By ROE+platform at CTAP operation      | Never in, public out via CTAP | Applet deselect OR as per CTAP standard     | Per applet select      |
| CTAP Platform shared secret      | 32-byte negotiated    | Communicate securely with a connected FIDO platform                   | Yes    | 256      | RAM                | ROE                    | By platform at CTAP operation          | Half in via CTAP, never out   | Applet deselect OR as per CTAP standard     | N/A                    |
| CTAP pinToken                    | 32-byte random value  | Allow or deny CTAP operations that require a PIN                      | No     | N/A      | RAM                | ROE                    | By ROE at CTAP operation               | CTAP                          | Applet deselect OR as per CTAP standard     | Per applet select      |
| CTAP permissions byte            | 1-byte stored value   | Check permissions of pinToken                                         | No     | N/A      | RAM                | ROE                    | By ROE at CTAP operation               | Never                         | Applet deselect OR as per CTAP standard     | Per applet select      |
| PIN retry counter                | 1-byte stored value   | Count number of times a PIN may be used before locking                | No     | N/A      | Flash              | ROE                    | By ROE at CTAP operation               | Never in, CTAP out            | CTAP reset OR correct PIN entry             | Per authenticator      |
| PIN retry power-off bit          | 1-bit stored value    | Require a power-off before a PIN may be used again                    | No     | N/A      | RAM                | ROE                    | By ROE at CTAP operation               | Never in, CTAP out            | Authenticator power off                     | Per authenticator boot |
| PIN change required bit          | 1-bit stored value    | Require a PIN change before a CTAP PIN/UV token will be issued        | No     | N/A      | Flash              | ROE                    | By platform                            | CTAP                          | CTAP pin set/change or CTAP reset           | N/A                    | 
| PIN minimum length               | 1-byte stored value   | Minimum allowable PIN length in UTF-8 codepoints                      | No     | N/A      | Flash              | ROE                    | By platform                            | CTAP                          | CTAP setMinPinLength or CTAP reset          | N/A                    |
| CTAP signature counter           | 4-byte stored value   | Ensures every signature is different; wear-leveled across 64 bytes    | No     | N/A      | Flash              | ROE                    | By ROE at applet install/reset         | Never in, CTAP out            | CTAP reset                                  | Per authenticator      |
| NFC lock state                   | 1-bit stored value    | Prevents authenticator from being used after NFC power-off requested  | No     | N/A      | RAM                | ROE                    | By ROE during NFC operation            | NFC in, never out             | Authenticator power off                     | Per authenticator boot |
| Attestation Private Key          | ECDSA key on P-256    | Generate attestation signatures                                       | Yes    | 128      | Flash              | ROE                    | By vendor when installing applet       | Vendor[1] in, never out       | Never                                       | Shared                 |
| Non-Discoverable Credential Key  | ECDSA key on P-256    | Generate credential signatures                                        | Yes    | 128      | RAM/External       | ROE+AES256-HMAC-SHA256 | By ROE at credential creation          | CTAP in Cred                  | From RAM immediately after use              | Per credential         |
| CTAP HMAC extension UV bytes     | 32-byte random value  | Derive CTAP2.1 HMAC Extension key from cred with UV                   | No     | 256      | Flash              | ROE                    | By ROE at applet install/reset         | Never                         | CTAP reset                                  | Per authenticator      |
| CTAP HMAC extension non-UV bytes | 32-byte random value  | Derive CTAP2.1 HMAC Extension key from cred without UV                | No     | 256      | Flash              | ROE                    | By ROE at applet install/reset         | Never                         | CTAP reset                                  | Per authenticator      |
| Cred IV                          | 16-byte random value  | Ensure every credential has different stored encrypted blocks         | No     | N/A      | Flash/RAM/External | ROE                    | By ROE at credential creation          | CTAP in Cred                  | When credential deleted; from RAM after use | Per credential         |
| Discoverable Credential Key      | ECDSA key on P-256    | Same as non-discoverable credential private keys                      | Yes    | 128      | Flash/RAM/External | ROE+AES256-HMAC-SHA256 | By ROE at credential creation          | Never[2] in, CTAP out in cred | When credential deleted; from RAM after use | Per credential         |
| Discoverable Credential IVs      | 16-byte random values | Ensure every stored value has different encrypted blocks              | No     | N/A      | Flash              | ROE                    | By ROE at credential creation          | Never                         | When credential deleted                     | Per credential         |
| Discoverable Relying Party ID    | AES256(RP ID)         | Store truncated relying party ID (not hash)                           | No     | N/A      | Flash              | ROE+AES256             | By relying party                       | CTAP                          | When credential deleted                     | Per credential         |
| Discoverable RP ID length        | 1-byte stored value   | Length of valid portion of stored relying party ID                    | No     | N/A      | Flash              | ROE                    | By relying party                       | CTAP                          | When credential deleted                     | Per credential         |
| Discoverable User ID             | AES256(Username)      | Stored user ID for credential                                         | No     | N/A      | Flash              | ROE+AES256             | By relying party                       | CTAP                          | When credential deleted                     | Per credential         |
| Discoverable Username            | AES256(Username)      | Stored user name for credential                                       | No     | N/A      | Flash              | ROE+AES256             | By user / relying party                | CTAP                          | When credential deleted                     | Per credential         |
| Discoverable Public Key          | 64-byte stored        | Stored public key for credential as uncompressed X, Y points on P-256 | No     | N/A      | Flash              | ROE                    | By ROE at credential creation          | Never in, CTAP out            | When credential deleted                     | Per credential         |
| Discoverable CredBlob            | AES256(CredBlob)      | Stored CredBlob for CTAP2.1 CredBlob extension                        | No     | N/A      | Flash              | ROE+AES256             | By user / relying party                | CTAP                          | When credential deleted                     | Per credential         |
| Discoverable CredBlob Len        | 1-byte stored value   | Length of valid portion of stored CredBlob                            | No     | N/A      | Flash              | ROE                    | By user / relying party                | CTAP                          | When credential deleted                     | Per credential         |
| Discoverable Unique-RP flag      | 1-byte stored value   | Mark whether discoverable credential is only one for an RP            | No     | N/A      | Flash              | ROE                    | By ROE at credential creation/deletion | Never                         | When credential deleted                     | Per credential         |
| LargeBlob Key                    | 32-byte random value  | Allow CTAP largeBlob storage for platform                             | No     | N/A      | None               | Not stored             | By ROE at LargeBlob request            | Never in, CTAP out            | N/A                                         | Per credential         |

[1] The attestation private key is installed by the vendor during the authenticator provisioning process, prior
to first use
