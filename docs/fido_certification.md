...
| Credential Wrapping Non-UV Key   | 32-byte random value  | Encrypt/decrypt KeyHandles credProtect<3 or no PIN                    | Yes    | 256      | Flash              | ROE                    | By ROE at applet install/reset         | Never                         | CTAP reset                                  | Per authenticator      |
...
| CTAP Platform Agreement Key      | EC key on P-256       | ECDH-generate a shared secret between Authenticator and Platform      | Yes    | 128      | RAM                | ROE                    | By ROE at CTAP operation               | Never in, public out via CTAP | Applet deselect OR as per CTAP standard     | Per applet select      |
| CTAP Platform shared secret      | 32-byte negotiated    | Communicate securely with a connected FIDO platform                   | Yes    | 256      | RAM                | ROE                    | By ROE+platform at CTAP operation      | Half in via CTAP, never out   | Applet deselect OR as per CTAP standard     | N/A                    |
